...
data{
  ...
  matrix[N_missing, 2] missing_prior; // parameters of  prior distributions on missing data
...
transformed data{
  ...
  // calculation normalization constant for missing data
  // due to upper and lower limits
...
parameters{
  ...
  vector<lower=0>[K_mi_risk_factors] r1_local;
  vector<lower=0>[K_mi_risk_factors] r2_local; 
}
...
transformed parameters{
  ...
  real<lower=0> tau;
  vector<lower=0>[K_mi_risk_factors] lambda;
  vector[K_mi_risk_factors] w;
  tau = r1_global * sqrt(r2_global);
  lambda = r1_local .* sqrt(r2_local);
  w = logit_prob_MI_coeffs .* lambda*tau;
  ...
  prob_MI = inv_logit( logit_prob_MI + X_mi_risk_factors_imputed*w);  
  }
...
model{
  ...
  // construct half cauchy for tau (global sale parameter)
