...
Rscript scripts/plot_simulation_fit.R \
	...
	--fit fit/full_simulation_deep-phyloMI_.Rds \
...
Rscript scripts/plot_trace.R \
	...
	--fit fit/base_simulation_base_deep-phyloMI_.Rds \
	--plotParams config/tmp.tsv \
	--nRow 3
rm -rf config/tmp.tsv
...
Rscript scripts/plot_trace.R \
	...
	--fit fit/full_simulation_base_deep-phyloMI_.Rds \
	--plotParams config/tmp.tsv \
	--nRow 3
rm -rf config/tmp.tsv
...
Rscript scripts/plot_trace.R \
	...
	--fit fit/extended_simulation_deep-phyloMI_.Rds \
	--plotParams config/tmp.tsv \
	--nRow 4
rm -rf config/tmp.tsv
...
grep -v "prev. MIs" config/extended_simulation_plot_params.tsv | \
	grep -v "logit_prob_mi_fnr" | \
	grep -v "logit_prob_mi_fpr" | \
	grep -v "logit_prob_mi_coeffs\\[1\\]" | \
	grep -v "logit_prob_mi_coeffs\\[3\\]" | \
	grep -v "logit_prob_mi_coeffs\\[5\\]" | \
	grep -v "logit_prob_mi_coeffs\\[7\\]" | \
	grep -v "logit_prob_mi_coeffs\\[9\\]" | \
	> config/tmp.tsv
